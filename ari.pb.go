...
type ChannelData struct {
	...
	ChannelVars map[string]string `protobuf:"bytes,11,rep,name=channelvars" json:"channelvars,omitempty" protobuf_key:"bytes,1,opt,name=key,proto3" protobuf_val:"bytes,2,opt,name=value,proto3"`
}
...
func (m *ChannelData) MarshalTo(dAtA []byte) (int, error) {
	...
	if len(m.ChannelVars) > 0 {
		for k, _ := range m.ChannelVars {
			dAtA[i] = 0x5a
			i++
			v := m.ChannelVars[k]
			mapSize := 1 + len(k) + sovAri(uint64(len(k))) + 1 + len(v) + sovAri(uint64(len(v)))
			i = encodeVarintAri(dAtA, i, uint64(mapSize))
			dAtA[i] = 0xa
			i++
